function [dictionary,paramIndices,sdelT] = compile_SE_dictionary_Hargreaves(offsetList, minTR, minTE, nRuns, df, dictionaryParams)
%% Jack Allen
%
%% DICTIONARY
...
nTimeCoursePts = size(offsetList,1)*nRuns;
...
paramIndex3 = repmat(dictionaryParams(3,dictionaryParams(3,:)>0) ,1,sum(dictionaryParams(1,:)>0)*sum(dictionaryParams(2,:)>0));
...
for nEntry = find(paramIndex1>paramIndex2)
   ...
   dictionary(nEntry,:) = sim_SE_hargreaves(paramIndex1(nEntry), paramIndex2(nEntry), minTR, minTE, offsetList,nRuns,df);
