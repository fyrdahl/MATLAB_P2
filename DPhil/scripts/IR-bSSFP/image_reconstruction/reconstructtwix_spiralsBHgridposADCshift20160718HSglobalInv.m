...
FIDstart = 18470;
% Read data from file
filename20160718 = (['~/Documents/DPhil/data/TWIX/',date,'/meas_MID',num2str(MIDstart+scanIndex),'_JA_IR_bSSFP_fp_',num2str(ADCshift),'ADCGINV_FID',num2str(FIDstart+scanIndex)]);

twix_obj20160718 = mapVBVD(filename20160718);
twix_obj20160718.image.flagRemoveOS = 1; %remove factor of 2 oversampling along each trajectory
% squeeze the data to remove unnecessary dimensions
image_data20160718 = twix_obj20160718.image{''};
% image_data(400:end,:,:) = 0;
spiralkspacedata = double(squeeze(image_data20160718(:,channel,((3*nTRs)+1):((3*nTRs)+nTRs))));
