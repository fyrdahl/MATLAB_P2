...
channel = 1;
% Read data from file
filename20160416 = (['~/Documents/DPhil/data/TWIX/',date,'/meas_MID',num2str(727+scanIndex),'_JA_IR_bSSFP_fp_tr10_fa10_noinv_shift',num2str(ADCshift),'_FID',num2str(9496+scanIndex)]);
twix_obj20160416 = mapVBVD(filename20160416);
twix_obj20160416.image.flagRemoveOS = 1; %remove factor of 2 oversampling along each trajectory
% squeeze the data to remove unnecessary dimensions
image_data20160416 = twix_obj20160416.image{''};
% image_data(400:end,:,:) = 0;
spiralkspacedata = double(squeeze(image_data20160416(:,channel,((3*nTRs)+1):((3*nTRs)+nTRs))));
