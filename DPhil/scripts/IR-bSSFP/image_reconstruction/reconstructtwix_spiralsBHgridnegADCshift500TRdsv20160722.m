...
%
date = '20160722'; %date of scan
ADCshifts = [0,2,1,4:2:10];
ADCshift = 0;
...
spiralkspacelocations_x(max(TRrange)+1:end) = [];
spiralkspacelocations_x(1:(min(TRrange)-1)) = [];
spiralkspacelocations_y(max(TRrange)+1:end) = [];
spiralkspacelocations_y(1:(min(TRrange)-1)) = [];
...
filename20160722 = (['~/Documents/DPhil/data/TWIX/',date,'/meas_MID',num2str(MIDstart+scanIndex),'_JA_IR_bSSFP_fp_',num2str(abs(ADCshift)),'ADC_FID',num2str(FIDstart+scanIndex)]);
twix_obj20160722 = mapVBVD(filename20160722);
twix_obj20160722.image.flagRemoveOS = 1; %remove factor of 2 oversampling along each trajectory
% squeeze the data to remove unnecessary dimensions
image_data20160722 = twix_obj20160722.image{''};
%%
...
% Read data from file
spiralkspacedata = double(squeeze(image_data20160722(:,channel,TRrange)));
