...
for n = 1:6
TIimage(:,:,n) = read_avw([savingdir,'/data/NIfTI/',date,'_',ID,'/TIgroup/',date,'_00',num2str(TIseriesStart+(n-1)),'_',TIprotocol,num2str(TI(n)),'ms.nii']);
end
for n = 7:numel(TI)
TIimage(:,:,n) = read_avw([savingdir,'/data/NIfTI/',date,'_',ID,'/TIgroup/',date,'_0',num2str(TIseriesStart+(n-1)),'_',TIprotocol,num2str(TI(n)),'ms.nii']);
end

for n = 1:numel(TE)
TEimage(:,:,n) = read_avw([savingdir,'/data/NIfTI/',date,'_',ID,'/TEgroup/',date,'_0',num2str(TEseriesStart+(n-1)),'_',TEprotocol,'_',num2str(TE(n)),'ms.nii']);
end
