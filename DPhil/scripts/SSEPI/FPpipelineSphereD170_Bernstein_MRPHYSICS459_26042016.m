...
offsets = fingerprintLists(:,:,12);

if strcmp(plotFlag,'noPlot') ==0
    plot_FP_parameters(offsets);
end
...
if strcmp(plotFlag,'noPlot') ==0
    ...
    plot_sim_comparison_bernstein(FPdata, T1, T2, offsets,nRuns,df);
end
...
%% Create dictionary
df = 0;
nTimeCoursePts = nRuns*size(offsets,1); % How many timepoints (e.g. 48 timecourse points for 2 cycles through a list of 24 entries)
[dictionaryOL18PL6BernMRPHYSICS459,dictionaryOL18PL6BernMRPHYSICS459_elapsedTime] = compile_SE_dictionary_Bernstein(offsets, nTimeCoursePts, nSlices, df, dictionaryParams);
